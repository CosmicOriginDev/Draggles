...
scream=LoadSound("MOD/sounds/monster-scream.ogg")
...
function hoverUpright()
	...
	for a = 1, 3, 2 do
		...
		f = f * robot.mass
		--f = 10000
		ConstrainAngularVelocity(robot.body, hover.hitBody, axes[a], v)
	end
end
...
function hoverTurn()
	...
	ConstrainAngularVelocity(robot.body, hover.hitBody, robot.axes[2], angVel)
end
...
function navigationUpdate(dt)
	...
	if GetPathState() ~= "busy" and #navigation.path == 0 and not navigation.hasNewTarget then
		if GetPathState() == "done" or GetPathState() == "idle" then
			...
			MakeHole(robot.bodyCenter,10,10,10)
		end
	end
end
...
function navigationMove(dt)
	if #navigation.path > 0 then
		...
		if navigation.unblock > 0 then
			MakeHole(robot.bodyCenter,5,5,5)
			robot.speed = -2
			...
			if navigation.unblockTimer>5 then
				leap()
			end
		end
	end
...
function update(dt)
	...
	if HasTag(robot.body, "inactive") then
		...
		if robot.inactive then
			...
			teleport(VecAdd(GetPlayerCameraTransform().pos,Vec(math.random(-100,100),10,math.random(-100,100))))
	...
	aimsUpdate(dt)
	if robot.distToPlayer<3.0 then		
		SetPlayerHealth(GetPlayerHealth()-0.03)
		PlaySound(scream,robot.bodyCenter,100)
		local mouth=FindBody("aim")
		local mouthPos=GetBodyTransform(mouth).pos
		local mouthRot=GetBodyTransform(mouth).rot
		SetCameraTransform(Transform(VecAdd(mouthPos,QuatRotateVec(mouthRot,Vec(0,0.25,GetPlayerHealth()*-2))),QuatLookAt(GetCameraTransform().pos,VecAdd(mouthPos,Vec(0,1,0)))),true)
		SetBodyVelocity(robot.body,Vec(0,0,0))
		SetPlayerVelocity(Vec(0,0,0))
		if GetPlayerHealth()==0 then
			leapAway()--prevents spawn camping
		end
	end
...
function summonWhenDragonIsDead()
...
function leap()
	if hover.contact>0 or IsPointInWater(robot.bodyCenter) then
		robot.stunned=1
		SetBodyVelocity(robot.body,Vec(0,0,0))
		ApplyBodyImpulse(robot.body,robot.bodyCenter,Vec(robot.mass*robot.dirToPlayer[1]*15,robot.mass*math.random(1,15),robot.mass*robot.dirToPlayer[3]*15))
	end
end
function tackle()
	...
	if hover.contact>0 or IsPointInWater(robot.bodyCenter) then
		...
		robot.stunned=2
		SetBodyVelocity(robot.body,Vec(0,0,0))
		ApplyBodyImpulse(robot.body,robot.bodyCenter,Vec(robot.mass*robot.dirToPlayer[1]*15,robot.mass*math.random(1,4),robot.mass*robot.dirToPlayer[3]*15))
	end
